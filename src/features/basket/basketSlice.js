...
const basketSlice = createSlice({
    ...
    reducers: {
        addProductToBasket: (state, action) => {
            ...
            console.log(ind);
            if (ind === -1) {
                let prod = { ...action.payload, qty: 1,sum:action.payload.price}
                console.log(prod);
                state.basketArr.push(prod)
            }
            else {
                state.basketArr[ind].qty = state.basketArr[ind].qty + 1;
                state.basketArr[ind].sum = state.basketArr[ind].sum + action.payload.price;
        ...
        increaseQty: (state, action) => {
            ...
            if (ind != -1) {
                ...
                let y = state.basketArr[ind].sum;
                state.basketArr[ind].qty = x + 1;
                console.log(state.basketArr[ind].qty);
                state.basketArr[ind].sum = y + action.payload.price;
                state.totalCnt = state.totalCnt + 1;
                state.totalSum = state.totalSum + action.payload.price;
            }
        },
